...
$kcadm create client-scopes/${SCOPE_ID}/protocol-mappers/models \
    ...
    -s config='{"aggregate.attrs": true, "multivalued": true, "userinfo.token.claim": true, "user.attribute": "policy", "id.token.claim": true, "access.token.claim": true, "claim.name": "policy"}'
...
$kcadm create clients \
    ...
    -s 'rootUrl=${authBaseUrl}' \
    -s 'baseUrl="/realms/werkstatt-hub/account/"' \
    -s directAccessGrantsEnabled=true \
    -s 'clientAuthenticatorType=client-secret' \
    -s 'webOrigins=["*"]' \
    -s 'redirectUris=["*"]' \
