...
@pytest.fixture()
def patch_failed_http_request(monkeypatch, keycloak_url, workshop_realm):
    def mock_get(url):
        assert url == f"{keycloak_url}/realms/{workshop_realm}"
        return httpx.Response(
            status_code=400, request=httpx.Request(url=url, method="GET")
        )
...
class TestKeycloak:
    ...
    def test_configure_fails(
        self, keycloak_url, workshop_realm, patch_failed_http_request
    ):
